...
def build_pedes_data(config):
    ...
    val_transform = transforms.Compose([
        transforms.Resize(size, interpolation=transforms.InterpolationMode.BICUBIC, antialias=True),  # 确保输入的图像具有一致的尺寸并减小失真
        transforms.ToTensor(),
        normalize
    ])
    ...
    rand_from = [
        ...
        transforms.RandomResizedCrop(size, (0.9, 1.0)),
    ...
    test_sample = config.experiment.test_sample
    test_sample_p = config.experiment.test_sample_p
    train_dataset = ps_train_dataset(config.anno_dir, config.image_dir, aug, aug_ss, split='train', max_words=77)
    test_dataset = ps_eval_dataset(config.anno_dir, test_sample, test_sample_p, config.image_dir, val_transform, split='test', max_words=77)
