...
from misc.utils import mixup_feathers
...
class CLIP(nn.Module):
    ...
    def forward(self, input, alpha):
        ...
        # img_id = input['img_id']
        # train sample
        if self.config.experiment.train_sample:
            for i in range(len(texts)):
                if random.random() < self.config.experiment.train_sample_p:
                    texts[i] = texts_bt[i]
        ...
        if self.config.experiment.mlm:
            ...
            text_ori_tokens = tokenize(texts, context_length=self.config.experiment.text_length).to(self.config.device)
            # mixup
            if self.config.experiment.mixup:
                text_aug_tokens = tokenize(texts_bt, context_length=self.config.experiment.text_length).to(self.config.device)
                text_aug_features, text_seq_embeddings = self.encode_text(text_aug_tokens, return_dense=True)

        ids = input['id'].to(self.config.device)

        image_features, image_seq_embeddings = self.encode_image(images, return_dense=True)
        text_ori_features, text_seq_embeddings = self.encode_text(text_ori_tokens, return_dense=True)


        if self.config.experiment.mixup:
            if random.random() < self.config.experiment.mixup_γ:
                text_features = text_ori_features

            else:
                text_features = mixup_feathers(text_ori_features, text_aug_features)
        else:
            text_features = text_ori_features
        ...
        with torch.no_grad():
            ...
            if self.config.experiment.mixup:
                text_features_ori = self.encode_text(text_ori_tokens).detach()
                text_features_aug = self.encode_text(text_aug_tokens).detach()
                if random.random() < self.config.experiment.mixup_γ:
                    text_features_s = text_ori_features
                else:
                    text_features_s = mixup_feathers(text_features_ori, text_features_aug)

            else:
                text_features_s = self.encode_text(text_ori_tokens).detach()
